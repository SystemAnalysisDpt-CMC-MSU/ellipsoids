classdef mlunit_test_disp < mlunitext.test_case
    ...
    methods
        ...
        function self = test_plotts(self)
            ...
            smartdb.disp.plotts(relObj,'xfieldname',...
                {'time_stamp_ts','time_stamp_ts'},...
        ...
        %
        function test_RelDataPlotterJointAndSuccessiveConsist(self)
            import modgen.common.*;
            POS_TOLERANCE=1e-4;
            plJointObj=process(true);
            SJoint=plJointObj.getPlotStructure();
            ...
            mlunitext.assert_equals(true,isequal(...
                figureKeyList,SJoint.figHMap.keys));
            mlunitext.assert_equals(true,isequal(...
                figureKeyList,SSep.figHMap.keys));
            mlunitext.assert_equals(true,isequal(figureKeyList,...
                SJoint.figToAxesToHMap.keys));
            mlunitext.assert_equals(true,isequal(figureKeyList,...
                SSep.figToAxesToHMap.keys));
            mlunitext.assert_equals(true,isequal(figureKeyList,...
                SJoint.figToAxesToPlotHMap.keys));
            mlunitext.assert_equals(true,isequal(figureKeyList,...
                SSep.figToAxesToPlotHMap.keys));
            for iFigure=1:nFigures
                figureKey=figureKeyList{iFigure};
                hFigureJoint=SJoint.figHMap(figureKey);
                hFigureSep=SSep.figHMap(figureKey);
                mlunitext.assert_equals(hFigureJoint.Name,hFigureSep.Name);
                mlunitext.assert_equals(hFigureJoint.Position,...
                    hFigureSep.Position);
                jointAxesMap=SJoint.figToAxesToHMap(figureKey);
                sepAxesMap=SJoint.figToAxesToHMap(figureKey);
                jointPlotMap=SJoint.figToAxesToPlotHMap(figureKey);
                sepPlotMap=SJoint.figToAxesToPlotHMap(figureKey);
                axesKeyList=jointAxesMap.keys;
                mlunitext.assert_equals(true,isequal(axesKeyList,...
                    sepAxesMap.keys));
                mlunitext.assert_equals(true,isequal(axesKeyList,...
                    jointPlotMap.keys));
                mlunitext.assert_equals(true,isequal(axesKeyList,...
                    sepPlotMap.keys));
                nAxes=numel(axesKeyList);
                for iAxes=1:nAxes
                    axesKey=axesKeyList{iAxes};
                    hAxesJoint=jointAxesMap(axesKey);
                    hAxesSep=sepAxesMap(axesKey);
                    mlunitext.assert_equals(hAxesJoint.XLim,hAxesSep.XLim);
                    mlunitext.assert_equals(hAxesJoint.YLim,hAxesSep.YLim);
                    mlunitext.assert_equals(hAxesJoint.ZLim,hAxesSep.ZLim);
                    posJointVec=hAxesJoint.Position;
                    posSepVec=hAxesSep.Position;
                    mlunitext.assert_equals(posJointVec(1:2),...
                        posSepVec(1:2));
                    jointPlotCVec=num2cell(jointPlotMap(axesKey));
                    sepPlotCVec=num2cell(sepPlotMap(axesKey));
                    mlunitext.assert_equals(numel(jointPlotCVec),...
                        numel(sepPlotCVec));
                    [jointTypeCVec,~,indJointBackVec]=unique(...
                        cellfun(@(x)x.Type,jointPlotCVec,...
                        'UniformOutput',false));
                    [sepTypeCVec,~,indSepBackVec]=unique(...
                        cellfun(@(x)x.Type,sepPlotCVec,...
                        'UniformOutput',false));
                    mlunitext.assert_equals(true,isequal(jointTypeCVec,...
                        sepTypeCVec)&&isequal(sort(indJointBackVec),...
                        sort(indSepBackVec)));
                    nTypes=numel(jointTypeCVec);
                    for iType=1:nTypes
                        curJointPlotCVec=reshape(jointPlotCVec(...
                            indJointBackVec==iType),[],1);
                        curSepPlotCVec=reshape(jointPlotCVec(...
                            indSepBackVec==iType),[],1);
                        switch jointTypeCVec{iType}
                            case 'light'
                            case 'text'
                                [isTextVec,indTextVec]=...
                                    ismemberbyfunc(...
                                    cellfun(@(x)x.Position,...
                                    curJointPlotCVec,...
                                    'UniformOutput',false),...
                                    cellfun(@(x)x.Position,...
                                    curSepPlotCVec,...
                                    'UniformOutput',false),...
                                    @(x,y)max(abs(x-y))<=POS_TOLERANCE);
                                mlunitext.assert_equals(true,...
                                    all(isTextVec)&&...
                                    ~any(diff(sort(indTextVec))==0));
                                mlunitext.assert_equals(true,isequal(...
                                    cellfun(@(x)x.String,...
                                    curJointPlotCVec,...
                                    'UniformOutput',false),...
                                    cellfun(@(x)x.String,...
                                    reshape(curSepPlotCVec(...
                                    indTextVec),[],1),...
                                    'UniformOutput',false)));
                            otherwise
                                propNameCVec={'DisplayName',...
                                    'XData','YData','ZData',...
                                    'CData','Color','Marker'};
                                propNameCVec=propNameCVec(cellfun(...
                                    @(x)isprop(curJointPlotCVec{1},x),...
                                    propNameCVec));
                                nProps=numel(propNameCVec);
                                if nProps > 0
                                    nObjs=numel(curJointPlotCVec);
                                    jointPropValCMat=cell(nObjs,nProps);
                                    sepPropValCMat=cell(nObjs,nProps);
                                    for iProp=1:nProps
                                        propName=propNameCVec{iProp};
                                        jointPropValCMat(:,iProp)=...
                                            cellfun(@(x)x.(propName),...
                                            curJointPlotCVec,...
                                            'UniformOutput',false);
                                        sepPropValCMat(:,iProp)=...
                                            cellfun(@(x)x.(propName),...
                                            curSepPlotCVec,...
                                            'UniformOutput',false);
                                    end
                                    [isObjVec,indObjVec]=...
                                        modgen.common.ismemberjoint(...
                                        num2cell(jointPropValCMat,1),...
                                        num2cell(sepPropValCMat,1),1);
                                    mlunitext.assert_equals(true,...
                                        all(isObjVec)&&...
                                        ~any(diff(sort(indObjVec))==0));
                                end
                        end
                    end
                end
            ...
            %
            function plOutObj=process(isJoint,varargin)
                plOutObj=smartdb.disp.RelationDataPlotter(...
                    'nMaxAxesRows',1,'nMaxAxesCols',3,...
                    'figureGroupKeySuffFunc',@(x)sprintf('_gr%d',x),...
                    varargin{:});
                ...
                if isJoint
                    ...
                    processRel('init');
                    plotSimple(@(rel)processRel('selectnext',rel));
                    SRelInfo=processRel('getinfo');
                    nTuples=SRelInfo.nTuples;
                    for iTuple=2:nTuples
                        plotSimple(@(rel)processRel('selectnext',rel));
                    end
                end
            end
            function res=processRel(funcName,rel)
                persistent SRelInfo;
                ...
                switch funcName
                    case 'getinfo'
                        res=SRelInfo;
                    case 'selectnext'
                        res=rel.getTuples(SRelInfo.indCurTuple);
                        SRelInfo.indCurTuple=SRelInfo.indCurTuple+1;
                    otherwise
                        modgen.common.throwerror('wrongInput',...
                            'funcName is unknown: %s',funcName);
                end
            end
        end
        ...
        function [fSimpleMasterCheck,fCheckPlots,fCheckDisplayedPlots,...
            ...
            %%
            function plotSimple(fRelGet)
                if nargin>0
                    inpRel=fRelGet(rel);
                else
                    inpRel=rel;
                end
                plObj.plotGeneric(inpRel,@figureGetGroupNameFunc,...
            ...
            function plotSimple2(fRelGet)
                if nargin>0
                    inpRel=fRelGet(rel);
                else
                    inpRel=rel;
                end
                plObj.plotGeneric(inpRel,@(varargin)'figure',...
                    {},@figureSetPropFunc,...
            ...
            function hVec=plotCreatePlot3Func(hAxes,color,~,tVec,varargin)
                ...
                hVec=plot3(tVec,sin(tVec),cos(tVec),...
                    ...
                    'Color',color);
                hVec=[hVec,xlabel(hAxes,'sin(t)')];
                hVec=[hVec,ylabel(hAxes,'cos(t)')];
                hVec=[hVec,zlabel(hAxes,'t')];
                grid(hAxes,'on');
                hold(hAxes,'on');
            end
            function hVec=axesSetPropDoNothingFunc(hAxes,basicAxesName,~,axesName)
                axis(hAxes,'auto');
                view(hAxes,3);
                hVec=title(hAxes,[basicAxesName,'_',axesName]);
            ...
            function hVec=axesSetPropFunc(hAxes,basicAxesName,axesNum,axesName)
                ...
                %
                hX=xlabel(hAxes,xLabel,'Interpreter',...
                    'tex');
                hY=ylabel(hAxes,yLabel,'Interpreter',...
                    'tex');
                hZ=zlabel(hAxes,zLabel,'Interpreter',...
                    'none');
                %
                hTitle=title(hAxes,[basicAxesName,'_',axesName],...
                    'Interpreter',...
                    'tex');
                %
                view(hAxes,3);
                ...
                h2=camlight('head');
                set(h2,'Parent',hAxes);
                hVec=[h1,h2,hX,hY,hZ,hTitle];
            end
            %
            function figureSetPropFunc(hFigure,figureName,indFigureGroup)
                set(hFigure,'NumberTitle','off','RendererMode',...
                    'manual','Renderer','OpenGL',...
                    'Name',figureName);
                if usejava('swing')
