...
if isNullReplacementSpec
    if ~(ismatrix(nullReplacementCMat)&&size(nullReplacementCMat,1)<=2&&...
...
if isNullReplacementSpec
    for iField=1:nFields
        if ~isReplacedByDefaultVec(iField)
            if ~modgen.common.checksize(nullReplacementCMat{1,iField},...
                    valueSizeMat(iField,:))
    ...
    if ~isIsNullReplacementSpec
        ...
        for iField=1:nFields
            fieldMetaData.checkFieldValue(iField,[false true],...
                'replaceNull',[true,false,false],nullReplacementCMat(:,iField));
            nullReplacementCMat(:,iField)=...
                fieldMetaData.reconstructFieldValues(iField,...
                nullReplacementCMat(:,iField),...
        ...
        for iField=1:nFields
            if ~isReplacedByDefaultVec(iField)
                if ~modgen.common.checksize(nullReplacementCMat{2,iField},...
                        valueSizeMat(iField,:));
                ...
                fieldMetaData.checkFieldValue(iField,[true true],...
