function [isOk,SValueTypeInfo]=checkvaluematchisnull(minDimensionality,...
    isSpecified,value,isNull,isValueNull)
% CHECKVALUEMATCHISNULL checks that the types and sizes of the value,isNull
%   and isValueNull matrices are consistent
%
% Input:
%   regular
...
if ~(islogical(isSpecified)&&modgen.common.isrow(isSpecified))
    if isempty(isSpecified)
        ...
        throwerror('wrongInput',...
            ['isSpecified parameter is expected to be a logical ',...
            'row-vector']);
    end
end
...
if isSpecified(3)
    ...
    isOk=islogical(isValueNull)&&...
        modgen.common.checksize(isValueNull,minDimensionSizeVec);
    %
    if ~isOk
        return;
    end
...
%
if isSpecified(2)
    isOk=smartdb.cubes.ACubeStructFieldType.checkvaluematchisnull_aux(...
        value,isNull);
end