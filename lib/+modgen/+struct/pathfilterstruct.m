...
%
    ...
    %
    ResStruct=modgen.struct.fieldfilterstruct(InpStruct,field2KeepList);
    %
    ...
    for iField=1:nUniqueFields
        ...
        if size(subFieldSeqCell,2)>1
            ...
            ResCell=num2cell(modgen.struct.pathfilterstruct(InpSubStruct,...
                subPath2KeepList));
            ResSubStruct=struct(fieldName,ResCell);
            %
            ResStruct=modgen.struct.binaryunionstruct(ResStruct,...
                ResSubStruct,@(x,y)y);
        end
    end
end
