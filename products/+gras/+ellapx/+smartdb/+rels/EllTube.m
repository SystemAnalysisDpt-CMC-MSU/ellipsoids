classdef EllTube<gras.ellapx.smartdb.rels.TypifiedByFieldCodeRel&...
    ...
    methods
        function catEllTubeRel = cat(self, newEllTubeRel)
            ...
            FIELDS_NOT_TO_CAT = {'approxSchemaDescr';...
                'approxSchemaName'; 'approxType'; 'calcPrecision';...
                'dim'; 'indSTime'; 'lsGoodDirNorm'; 'lsGoodDirVec';...
                'sTime'; 'scaleFactor'; 'xsTouchOpVec'; 'xsTouchVec'};
            fieldsToCatVec =...
                setdiff(fieldnames(SDataFirst), FIELDS_NOT_TO_CAT);
            catRes = cellfun(@(field) cellfun(@(first, second)...
                cat(ndims(first), first, second), SDataFirst.(field),...
                SDataSecond.(field), 'UniformOutput', false),...
                fieldsToCatVec, 'UniformOutput', false);
            notCatRes = cellfun(@(field) SDataFirst.(field),...
                FIELDS_NOT_TO_CAT, 'UniformOutput', false);
            CatFunResult = cell2struct([catRes; notCatRes],...
                [fieldsToCatVec; FIELDS_NOT_TO_CAT]);
            catEllTubeRel = ...
                gras.ellapx.smartdb.rels.EllTube.fromStructList(...
                'gras.ellapx.smartdb.rels.EllTube', {CatFunResult});
        end
        function cutEllTubeRel = cut(self, cutTimeVec)
            function cutResObj = getCutObj(whatToCutObj, isCutTimeVec)
                %isCutTimeVec = isCutTimeCVec{1};
                if ndims(whatToCutObj) == 1
                    cutResObj = whatToCutObj(isCutTimeVec);
                elseif ndims(whatToCutObj) == 2
                    cutResObj = whatToCutObj(:, isCutTimeVec);
                elseif ndims(whatToCutObj) == 3
                    cutResObj = whatToCutObj(:, :, isCutTimeVec);
                end
            end
            %
            ...
            %newTimeVec = SData.timeVec{1}(isSysNewTimeIndVec);
            % need to change
            FIELDS_NOT_TO_CUT = {'approxSchemaDescr';...
                'approxSchemaName'; 'approxType'; 'calcPrecision';...
                'dim'; 'indSTime'; 'lsGoodDirNorm'; 'lsGoodDirVec';...
                'sTime'; 'scaleFactor'; 'xsTouchOpVec'; 'xsTouchVec'};
            fieldsToCutVec =...
                setdiff(fieldnames(SData), FIELDS_NOT_TO_CUT);
            cutRes = cellfun(@(field) cellfun(@(cutObj)...
                getCutObj(cutObj, isSysNewTimeIndVec), SData.(field),...
                'UniformOutput', false),...
                fieldsToCutVec, 'UniformOutput', false);
            notCutRes = cellfun(@(field) SData.(field),...
                FIELDS_NOT_TO_CUT, 'UniformOutput', false);
            CutFunResult = cell2struct([cutRes; notCutRes],...
                [fieldsToCutVec; FIELDS_NOT_TO_CUT]);
            cutEllTubeRel = ...
                gras.ellapx.smartdb.rels.EllTube.fromStructList(...
                'gras.ellapx.smartdb.rels.EllTube', {CutFunResult});
            
            
            
%             for iTuple = 1 : nTuples
%                 SData.timeVec{iTuple} =...
%                     SData.timeVec{iTuple}(isSysNewTimeIndVec);
%                 SData.QArray{iTuple} =...
%                     SData.QArray{iTuple}(:, :, isSysNewTimeIndVec);
%                 SData.aMat{iTuple} =...
%                     SData.aMat{iTuple}(:, isSysNewTimeIndVec);
%                 SData.MArray{iTuple} =...
%                     SData.MArray{iTuple}(:, :, isSysNewTimeIndVec);
%                 SData.ltGoodDirMat{iTuple} =...
%                     SData.ltGoodDirMat{iTuple}(:, isSysNewTimeIndVec);
%                 SData.ltGoodDirNormVec{iTuple} =...
%                     SData.ltGoodDirNormVec{iTuple}(isSysNewTimeIndVec);
%                 SData.xTouchCurveMat{iTuple} =...
%                     SData.xTouchCurveMat{iTuple}(:, isSysNewTimeIndVec);
%                 SData.xTouchOpCurveMat{iTuple} =...
%                     SData.xTouchOpCurveMat{iTuple}(:, isSysNewTimeIndVec);
%             end
%             cutEllTubeRel = ...
%                 gras.ellapx.smartdb.rels.EllTube.fromStructList(...
