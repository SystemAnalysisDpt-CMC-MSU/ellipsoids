    ...
    methods (Static)
        function ellTubeRel=fromQArrays(QArrayList,aMat,varargin)
            ...
            MArrayList=cellfun(@(x)zeros(size(x)),QArrayList,...
                'UniformOutput',false);
            ellTubeRel=EllTube.fromQMArrays(QArrayList,aMat,MArrayList,varargin{:});
        end
        function ellTubeRel=fromQMArrays(QArrayList,aMat,MArrayList,...
            ...
            import gras.ellapx.smartdb.rels.EllTubeBasic;
            %
            nObj = length(QArrayList);
            QArrayInterpObjList = cell(1,nObj);
            for indObj = 1:nObj
                QArrayInterpObjList{indObj} = ...
                    gras.interp.MatrixSysNearestInterp(...
                    [QArrayList(indObj) MArrayList(indObj)],varargin{1});
            end
            aMatInterpObj = ...
                gras.mat.interp.MatrixNearestInterp(aMat,varargin{1});
            ltGoodDirInterObj = gras.mat.interp.MatrixNearestInterp(varargin{2},varargin{1});
        ...
        function ellTubeRel = fromEllMArray(qEllArray, ellMArr, varargin)
            ...
            %
            ellTubeRel=EllTube.fromQMArrays({qArray},aMat,{ellMArr},varargin{:});
