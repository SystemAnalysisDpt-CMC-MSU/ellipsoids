    ...
    methods
        ...
        function plObj = plotExt(self,varargin)
            ...
            function [xCMat,fCMat] = fCalcBodyArr(~,varargin)
                dim = self.dim;
                allEllMat(size(self.QArray,1),size(self.timeVec{1},2))...
                    = ellipsoid;
...
%                 profile viewer
                if dim == 3
                    ...
                    allEll2Mat  = move2origin(allEllMat(:, end));
                    allEll2Mat  = allEll2Mat';
                    [lGridMat, ~] = allEll2Mat(1).calcGrid([1, 13]);                  
                    lGridMat = lGridMat';                  
                    nDim = size(lGridMat, 2);
                    mDim = size(allEll2Mat, 2);
                    xMat = zeros(3,nDim);
                    ...
                    for i = 1:nDim
                        ...
                        mval = ABS_TOL;
                        for j = 1:mDim
                            if trace(allEll2Mat(1, j)) > ...
                                    ABS_TOL;
                                qMat = double(inv(allEll2Mat(1, j)));
                                val = lVec' * qMat * lVec;
                                if val > mval
                                    mval = val;
                                end
                ...
                function  getEllMat(ind)
                    a2Mat = self.aMat{ind};
                    q2Mat = self.QArray{ind};
                    arrayfun(@(x) getEll2Mat(x),...
                    1:size(q2Mat,3),'UniformOutput', false);
                    
                    function getEll2Mat(ind2)
                       allEllMat(ind,ind2) = ellipsoid(a2Mat(:,ind2),q2Mat(:,:,ind2));
                    end
                end
            end
        end
