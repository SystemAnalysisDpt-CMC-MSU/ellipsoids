...
tStartGlobal=tic;
%% Parse input
[reg,isRegSpecVec,confRepoMgr,sysConfRepoMgr,ellApxBuilderName,isConfRepoMgrSpec,...
    isSysConfRepoMgrSpec]=parseparext(varargin,...
    {'confRepoMgr','sysConfRepoMgr','ellApxBuilderName';...
    [],[],'gras.ellapx.uncertcalc.EllApxBuilder';...
    @(x)isa(x,'gras.ellapx.uncertcalc.conf.IConfRepoMgr'),...
    @(x)isa(x,'gras.ellapx.uncertcalc.conf.sysdef.AConfRepoMgr'), ...
    @(x)ischar(x)},...
    [0,1], 'regCheckList',{'isstring(x)'},'regDefList',{''});
%
...
newObj = feval(ellApxBuilderName, confRepoMgr,pDynObj, goodDirSetObj);
[ellTubeRel,ellUnionTubeRel] = newObj.build();
%
