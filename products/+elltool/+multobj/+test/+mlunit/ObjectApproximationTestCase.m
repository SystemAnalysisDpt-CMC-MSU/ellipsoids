classdef ObjectApproximationTestCase < mlunitext.test_case
    ...
    methods
        ...
        function self = testIsConvexHull(self)
            ...
            [convexMat convexVec discrVec vertMat]= calcconvexhull(pointsMat,...
                [],[],3,{'relPrec',1.e-7,'errorCheckMode',0,'faceDist',.9e-6});
            ...
            [convexMat convexVec discrVec vertMat]= calcconvexhull(pointsMat,...
                [],[],3,{'approxPrec',1.e-4,'discardIneqMode',0,'inApproxDist',1.e-6});
            mlunitext.assert(all(all((expConvexMat - convexMat)<PREC))&&...
                all((expConvexVec - convexVec)<PREC)...
                &&all((expDiscrVec - discrVec)<PREC)&&...
                all(all((expVertMat - vertMat)<PREC)));
            ...
            expConvexMat = [0 0;-0.2169 0.9762;-0.9701 0.2425;-0.2588 0.9659;...
                            0.5145 -0.8575; 0.9659 -0.2588; -0.7071  -0.7071;...
                            0 0];
            expConvexVec = [0;-1.8439; 0;-1.8024; 0;-4.0532; 0; -1.0000];
            expDiscrVec =  1.0e-06*[0 0 0 0.0298 0 0.1788];
            expVertMat = [0 0;5 3; 0.5 2];
            [convexMat convexVec discrVec vertMat]= calcconvexhull(pointsMat,...
                [],[],2,{'inftyDef',1.e8,'incDim',1});
            mlunitext.assert(all(all((expConvexMat - convexMat)<PREC))&&...
                all((expConvexVec - convexVec)<PREC)...
                &&all((expDiscrVec - discrVec)<PREC)&&...
                all(all((expVertMat - vertMat)<PREC)));
            ...
            [convexMat convexVec discrVec vertMat]= calcconvexhull(pointsMat,...
                [],[],2,{'inApproxDist',1e-2,'ApproxDist',9e-1});
            mlunitext.assert(all(all((expConvexMat - convexMat)<PREC))&&...
                all((expConvexVec - convexVec)<PREC)...
                &&all((expDiscrVec - discrVec)<PREC)&&...
                all(all((expVertMat - vertMat)<PREC)));
            
            % the same points,but bad acuracy
            pointsMat = [1 1;0 2; 3 5; 1 6; 2 7; 0.5 8];
            ...
            [convexMat convexVec discrVec vertMat]= calcconvexhull(pointsMat,...
              [1 2 3],[1 1 1],3,{'ApproxDist',1.e-7,'precTest',1.e-7,'faceDist',1.e-8});
            mlunitext.assert(all(all((expConvexMat - convexMat)<PREC))&&...
                all((expConvexVec - convexVec)<PREC)...
                &&all((expDiscrVec - discrVec)<PREC)&&...
                all(all((expVertMat - vertMat)<PREC)));
            ...
            %wrong input
            self.runAndCheckError('elltool.multobj.calcconvexhull([1 2 3; 2 8 0],[],[],0)',...
                'wrongSize');
        end
        %
        function self = testIsEllipsoidApprox(self)
            ...
            [approxMat approxVec discrVec vertMat]= calcellipsoidapprox(semiaxesVec,...
                [],[],4,{'approxPrec',1e-5,'freeMemoryMode',0,'faceDist',.9e-8,...
                'relPrec',1.e-8});
            ...
            [approxMat approxVec discrVec vertMat]= calcellipsoidapprox(semiaxesVec,...
                 [1 2],[1 -1],5,{'discardIneqMode',0,'faceDist',1e-9,...
            ...
            [approxMat approxVec discrVec vertMat]= calcellipsoidapprox(semiaxesVec,...
                 [1 2],[1 -1],3,{'precTest',1e-2,'ApproxDist',1e-2,'faceDist',1e-3});
            ...
            %wrong input
            self.runAndCheckError('elltool.multobj.calcellipsoidapprox([1 3 5;1 5 2],[],[],0)',...
                'wrongSize');
            self.runAndCheckError('elltool.multobj.calcellipsoidapprox([1 -1 0],[],[1 4 9],0)',...
                'wrongSizes');
            properties={'relPrec','1e-6'};   %#ok<NASGU>
            self.runAndCheckError('elltool.multobj.calcellipsoidapprox([1 1 1],[],[],1,properties)',...
                'wrongParamsType');
        end
    end
end