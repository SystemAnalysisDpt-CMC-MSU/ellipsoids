...
PROP_LIST={'nAddTopElems','errorCheckMode','approxPrec',...
    'freeMemoryMode','discardIneqMode','incDim','faceDist',...
    'inApproxDist','ApproxDist','precTest','relPrec',...
    'inftyDef','isVerbose'};

controlParams=zeros(1,numel(PROP_LIST));
if(nPropExpected > 0)
    SParams=params.parseParams(properties,PROP_LIST);
    for iElem=1:numel(PROP_LIST)
        checkvar(SParams.(PROP_LIST{iElem}),@(x)isa(x,'double'),'errorTag',...
            'wrongParamsType','errorMessage','properties must be double');
        controlParams(iElem)=SParams.(PROP_LIST{iElem});
    end
    checkmultvar('(x1<x2)',2,SParams.('faceDist'),...
        SParams.('ApproxDist'), 'errorTag',...
        'wrongParams','errorMessage',...
        'faceDist must be less then ApproxDist');
else
    for iElem=1:numel(PROP_LIST)
        controlParams(iElem)=params.getPropValue(PROP_LIST{iElem});
    end
end
