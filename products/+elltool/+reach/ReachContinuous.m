    ...
    methods (Access = private)
        ...
        function ellTubeRel = makeEllTubeRel(self, smartLinSys, l0Mat,...
            ...
            try
                ...
                WR_INP = ':wrongInput';
                R_PROB = ':regProblem';
                R_DISABLED = ':RegIsDisabled';
                ONLY_CHECK = ':onlyCheckIsEnabled';
                LOW_REG_TOL = ':regTolIsTooLow';
                ODE_45_REG_TOL = ':Ode45Failed';
                BAD_CALC_PREC = ':BadCalcPrec';
                BAD_INIT_SET = ':BadInitSet';
                %
                R_PROB_STR = 'There is a problem with regularization. ';
                INIT_SET_PROB_STR = ['There is a problem with initial ',...
                    'set (x0Ell, second parameter). '];
                CALC_PREC_PROB_STR = ['There is a problem with ',...
                    'calculation precision. Try to do some of this:\n'];
                USE_REG_STR = ['Try to enable it: set property ',...
                    '''isRegEnabled'' to ''true'', ''isJustCheck'' to ',...
                    '''false'' and ''regTol'' to some positive.'];
                LOW_REG_TOL_STR = ['Try to increase regularization ',...
                    'tolerance: increase value of ''regTol'' property.'];
                SMALL_INIT_SET_STR = ['Try to increase it: change its ',...
                    'shape matrix'];
                BAD_TIME_VEC_STR = ['Try to decrease the length of ',...
                ...
                SECOND_COMMON_PART_BAD_ELL_STR =...
                    ...
                    'its shape matrix.'];
                BAD_CONTROL_STR = [FIRST_COMMON_PART_BAD_ELL_STR,...
                    'control', SECOND_COMMON_PART_BAD_ELL_STR];
                BAD_DIST_STR = [FIRST_COMMON_PART_BAD_ELL_STR,...
                    'disturbance', SECOND_COMMON_PART_BAD_ELL_STR];
                BAD_INIT_SET_STR = [FIRST_COMMON_PART_BAD_ELL_STR,...
                    'initial set', SECOND_COMMON_PART_BAD_ELL_STR];
                %
                if strcmp(meObj.identifier,...
                        'MODGEN:COMMON:CHECKVAR:wrongInput')
                    errorStr = [R_PROB_STR, USE_REG_STR];
                    errorTag = [WR_INP, R_PROB, ONLY_CHECK];
                elseif strcmp(meObj.identifier, 'MATLAB:badsubscript')
                    errorStr = [R_PROB_STR, LOW_REG_TOL_STR];
                    errorTag = [WR_INP, R_PROB, LOW_REG_TOL];
                elseif strcmp(meObj.identifier,...
                        'GRAS:ODE:ODE45REG:wrongState')
                    errorStr = [R_PROB_STR, LOW_REG_TOL_STR];
                    errorTag =...
                        [WR_INP, R_PROB, LOW_REG_TOL, ODE_45_REG_TOL];
                elseif strcmp(meObj.identifier,...
                        ['GRAS:ELLAPX:SMARTDB:RELS:',...
                        'ELLTUBETOUCHCURVEBASIC:',...
                        'CHECKTOUCHCURVEINDEPENDENCE:',...
                        'wrongInput:touchCurveDependency'])
                    errorStr = [CALC_PREC_PROB_STR, BAD_TIME_VEC_STR,...
                        BAD_CONTROL_STR, BAD_DIST_STR, BAD_INIT_SET_STR];
                    errorTag = [WR_INP, BAD_CALC_PREC];
                elseif strcmp(meObj.identifier,...
                        ['GRAS:ELLAPX:LREACHUNCERT:EXTINTELLAPXBUILDER',...
                        ':EXTINTELLAPXBUILDER:wrongInput'])
                    errorStr = [INIT_SET_PROB_STR, SMALL_INIT_SET_STR];
                    errorTag = [WR_INP, BAD_INIT_SET];
                elseif strcmp(meObj.identifier,...
                        ['GRAS:ELLAPX:LREACHUNCERT:EXTINTELLAPXBUILDER',...
                        ':CALCELLAPXMATRIXDERIV:wrongInput'])
                    errorStr = [R_PROB_STR, USE_REG_STR];
                    errorTag = [WR_INP, R_PROB, R_DISABLED];
                end
                if isempty(errorStr)
                    throw(meObj);
                else
                    currClassStr = modgen.common.getcallername();
                    currClassStr = upper(strrep(currClassStr, '.', ':'));
                    friendlyMeObj =...
                        MException([currClassStr, ':', errorTag],...
                        errorStr);
