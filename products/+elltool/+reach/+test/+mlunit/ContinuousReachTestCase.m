classdef ContinuousReachTestCase < mlunitext.test_case
    ...
    methods
        ...
        function self = testDynGetters(self)
            ...
            intEllTube=ellTubeRel.getTuplesFilteredBy('approxType', gras.ellapx.enums.EApproxType.Internal); % now there are only internal tubes in the table
            extEllTube=ellTubeRel.getTuplesFilteredBy('approxType',gras.ellapx.enums.EApproxType.External); % there are only external tubes in the table                    
            ...
            function isOk=auxtestProbDynGetters(ellTube,probDynamicsList,goodDirSetList)
                ...
                function isEqual=isEqualBPBandpDynBPB(probDynamicsList)
                    ...
                    function isCurEqual=isEqualBPBandpDynBPBTimeInterval(probDynObj,timeVec,iLinSys)
                        ...
                        iStep=1;
                        for iCurTime=timeVec
    %                         if (isBackward)
    %                           Are in this case linSysList sorted according to backward time?
    %                           Or should I find the new corresponding iLinSys
    %                           index in linSysList?
    %                           (sysTimeVecLenght-iLinSys+1)
    %                         end; 
                            pDynBPBMat=probDynObj.getBPBTransDynamics().evaluate(iCurTime);
                    ...
                    %
                    function isCurEqual=isEqualBPBandpDynBPBatCurTime(pDynBPBMat,iTime,iLinSys)                   
                        t=iTime;
