...
% modgen.common.checkvar(secObjArr,@(x)isa(x, 'ellipsoid') || isa(x, 'elltool.core.GenEllipsoid') || ...
%     isa(x, 'polytope'),'errorTag','wrongInput', 'errorMessage',...
%     'second input argument must be ellipsoid or polytope.');
modgen.common.checkvar(secObjArr,@(x)isa(x, 'polytope') || isa(x, 'ellipsoid') || isa(x, 'elltool.core.GenEllipsoid'), ...
...
if isa(secObjArr,'ellipsoid') || isa(secObjArr, 'elltool.core.GenEllipsoid')
