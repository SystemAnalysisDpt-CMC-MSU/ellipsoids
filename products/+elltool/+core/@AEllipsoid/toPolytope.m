...
%
ell = varargin{1}; %replaced isMe(x) to isa(x, 'ellipsoid')||isa(x, 'elltool.core.GenEllipsoid')
checkvar(ell, @(x) (isa(x, 'ellipsoid') || isa(x, 'elltool.core.GenEllipsoid')) ...
    && numel(x) == 1&&(dimension(x) == 3 || dimension(x) == 2), ...
    'errorTag', 'wrongInput', 'errorMessage', ...
    'First argument must be ellipsoid in 3D or 2D');
