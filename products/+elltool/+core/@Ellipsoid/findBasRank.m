...
% FINDBASrank - find basis of space that is linear hull of
%               input vector and find rank of that linear hull
% Input:
%   regular:
...
%   rank: double: [1,1] - rank of the convex hull of input vectors
%
...
rank = sum(abs(diag(rBasMat)) > tolerance);
rank = rank(1); %for case where rBasZMat is vector.
end