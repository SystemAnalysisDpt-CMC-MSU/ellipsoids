...
%
%
function ellDist = findEllEllDistance(ellObj1,ellObj2,nMaxIter,absTol,flag)
    import elltool.core.Ellipsoid
    diag1Vec=diag(ellObj1.diagMat);
    diag2Vec=diag(ellObj2.diagMat);
    isFirstInfinite=diag1Vec==Inf;
    isSecondInfinite=diag2Vec==Inf;
    if isFirstInfinite==isSecondInfinite 
        if  sum(isFirstInfinite)==0
            ellDist=computeEllEllDistance(ellObj1,ellObj2,nMaxIter,absTol,flag);
        else% consider infinite subspaces 
            newDiag1Vec=diag1Vec(~isFirstInfinite);
            newDiag2Vec=diag2Vec(~isSecondInfinite);
            newV1Mat=ellObj1.eigvMat(~isFirstInfinite,~isFirstInfinite);
            newV2Mat=ellObj2.eigvMat(~isSecondInfinite,~isSecondInfinite);
            newCen1Vec=ellObj1.centerVec(~isFirstInfinite);
            newCen2Vec=ellObj2.centerVec(~isSecondInfinite);
            ellDist=computeEllEllDistance(...
                Ellipsoid(newCen1Vec,newDiag1Vec,newV1Mat),...
                Ellipsoid(newCen2Vec,newDiag2Vec,newV2Mat),...
                nMaxIter,absTol,flag);
        end
    else
        throwerror('bad_situation','The algorithms is not invented yet. Please try again later');
    end   

%
function ellDist = computeEllEllDistance(ellObj1,ellObj2,nMaxIter,absTol,flag)
...
function [ distEllVec timeOfComputation ] = computeEllVecDistance(ellObj,vectorVec,nMaxIter,absTol, relTol)
    ...
    %
     ...
     elseif (vectorEllVal==1)
         ...
         oneStepError=Inf;
         kIterr=2;
         while( kIterr < nMaxIter ) && ( oneStepError > relTol )
             deltaF = fDetFunction(xVec(kIterr))-fDetFunction(xVec(kIterr-1));
             if abs(deltaF) <= absTol
                 throwerror('notSecant','Secant method is not applicable.');
             else
                 xVec(kIterr+1)=xVec(kIterr)-fDetFunction(xVec(kIterr))*...
                     (xVec(kIterr)-xVec(kIterr-1))/deltaF;
                 oneStepError=abs(xVec(kIterr)-xVec(kIterr-1))^2;
             end
             kIterr=kIterr+1;
         end
         lambda=xVec(kIterr);
...
function distEllEll = proceedEllEllDist(ellObj1, ellObj2, flag)
    %
    ...
    if (nEllObj1 > 1) && (nEllObj2 > 1)
        ...
        for i = 1:mSize1
            for j = 1:kSize1
                distEllEll(i,j)=findEllEllDistance(ellObj1(i,j),ellObj2(i,j),...
    ...
    elseif (nEllObj1 > 1)
        ...
        for i = 1:mSize1
            for j = 1:kSize1
                distEllEll(i,j)=findEllEllDistance(ellObj1(i,j),ellObj2,...
                    N_MAX_ITER,ABS_TOL,flag);      
            end
        end
    else
        ...
        for i = 1:mSize2
          for j = 1:kSize2        
            distEllEll(mSize2,kSize2)=findEllEllDistance(ellObj1,ellObj2(i,j),...
                N_MAX_ITER,ABS_TOL,flag);
          end
        end
    end
