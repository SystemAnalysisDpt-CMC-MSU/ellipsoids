classdef EllipsoidSpecialTC < elltool.core.test.mlunit.EllFactoryTC
...
%                 'testEllipsoid1Vec','testEllipsoid2Vec','testAnswVec','nEllVec');
        ...
        function self = testEllunionEa(self)
            ...
            ansEllVec = self.createEll([0.5, 0].', [2.389605510164642, ...
            ...
            resEllVec = ellunion_ea(testEllVec);
            ansEllVec = self.createEll([0.361900110249858, ...
            ...
            ansEllCenterVec = [0.678847905650305, 0.271345357930677, ...
                0.242812593977658].';
            ansEllVec = self.createEll(ansEllCenterVec, ansEllShape);
        ...
        function self = testEllEnclose(self)
            ...
            ansEllVec = self.createEll([0, 0].', eye(2));
            [isEq, reportStr] = isEqual(resEllVec, ansEllVec);
            ...
            resEllVec = ell_enclose(pointsVec);
            ansEllVec = self.createEll([0, 0].', [4, 0; 0, 1/9]);
            [isEq, reportStr] = isEqual(resEllVec, ansEllVec);
            mlunitext.assert_equals(true, isEq, reportStr);
            
            pointsVec = [1/2, 0, 0, 0; 0, 0, 0, -3];
            resEllVec = ell_enclose(pointsVec);
            ansEllVec = self.createEll([1/6, -1].', [1/9, 1/3; 1/3, 4]);
            [isEq, reportStr] = isEqual(resEllVec, ansEllVec);
            mlunitext.assert_equals(true, isEq, reportStr);
            ...
            resEllVec = ell_enclose(pointsVec);
            ansEllVec = self.createEll([0, 0, 0].', eye(3));
            [isEq, reportStr] = isEqual(resEllVec, ansEllVec);
            mlunitext.assert_equals(true, isEq, reportStr);
        ...
        function self = testMultiDimensionalConstructor(self)
            % one argument
            testShape = [2,0;0,3];
            testEll = self.createEll(testShape);
            testShMatArray = zeros(2,2,3,4);
            testShMatArray(:,:,1,3) = testShape;
            testEllArray = self.createEll(testShMatArray);
            mlunitext.assert(eq(testEllArray(1,3),testEll));
            % two arguments and properties
            testShape = [2,0;0,3];
            testCent = [1;5];
            testEll = self.createEll(testCent, testShape);
            testCentArray = zeros(2,3,4);
            testCentArray(:,1,3) = testCent;
            testEllArray1 = self.createEll(testCentArray, testShMatArray);
            testEllArray2 = self.createEll(testCentArray, testShMatArray, ...
                'absTol', 1e-3);
            ...
            testEllArray = self.createEll(testCentArray, testShMatArray);
            mlunitext.assert(eq(testEllArray(1),testEll));
            % bad dimensions
            self.runAndCheckError(...
                'self.createEll(zeros(3,4,5,6),zeros(3,3,5,5,6))',...
                'wrongInput');
            self.runAndCheckError(...
                'self.createEll(zeros(3,4,5,6,7,8),zeros(3,3,5,5,6))',...
                'wrongInput');
            self.runAndCheckError(...
                'self.createEll(zeros(3,4,5,6,7,8),zeros(3,3,5,5,6,6,6))',...
                'wrongInput');
            self.runAndCheckError(...
                'self.createEll(zeros(3),zeros(3))',...
                'wrongInput');
        end
