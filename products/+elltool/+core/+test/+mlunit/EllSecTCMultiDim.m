classdef EllSecTCMultiDim < elltool.core.test.mlunit.EllFactoryTC
    ...
    methods
        ...
        function self = testIsContainedInIntersection (self)
            ...
            test1EllArray = self.createObjectArray(array1Size, @(varargin)self.createEll(varargin{:}), ...
                [2; 1], [4, 1; 1, 1], 2);
            array2Size = [1, 1];
            test2EllArray = self.createObjectArray(array2Size, @(x)self.createEll(eye(x)), ...
                2, 1, 1);
            testCorrect(0, 'i', 1);
            testCorrect(1, 'i', 0);
            testCorrect(1, [], 0);
            ...
            test2EllArray = self.createObjectArray(array2Size, @(x)self.createEll(eye(x)), ...
                3, 1, 1);
            test2EllArray(1, 2, 1, 1, 1, 3, 1) = self.createEll([2; 1; 0], ...
                myMat);
            testCorrect(1, 'i', 1);
            testCorrect(1, 'u', 0);
            ...
            test2EllArray = self.createObjectArray(array2Size, @(x)self.createEll(eye(x)), ...
                3, 1, 1);
            test2EllArray(1, 2, 1, 1, 1, 1, 1) = self.createEll([5; 5; 5], ...
                myMat);
            testCorrect(1, 'i', -1);
            testCorrect(1, 'u', 0);
            ...
            test2EllArray = self.createObjectArray(array2Size, @(x)self.createEll(eye(x)), ...
                4, 1, 1);
            test2EllArray(1, 2, 1, 1, 1, 1, 1) = self.createEll([5; 5; 5; 5], ...
            ...
            test2EllArray = self.createObjectArray(array2Size, @(varargin)self.createEll(varargin{:}), ...
                ones(100, 1), diag([0.2 * ones(1, 50), 0.5 * ones(1, 50)]), 2);
            test2EllArray(1, 2, 1, 1, 1, 2, 1) = self.createEll(-ones(100, 1), ...
    ...
    methods
        function [varargout] = createTypicalArray(self, flag)
            switch flag
                case 1
                    ...
                    my1Ell = self.createEll(diag( 4 * ones(1, 10)));
                    my2Ell = self.createEll(eye(10));
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        10, 1, 1);
                    myMat = eye(10);
                    ansEllMat = diag(13 ^ 2 * ones(1, 10));
                    ansEllVec = self.createObjectArray([1, 10], @(varargin)self.createEll(varargin{:}), ...
                        ansEllMat, 1, 1);
                    varargout{1} = my1Ell;
                    varargout{2} = my2Ell;
                ...
                case 2
                    ...
                    my1Ell = self.createEll(10 * ones(7, 1), diag(9 * ones(1, 7)));
                    my2Ell = self.createEll(-3 * ones(7, 1), diag([4, ones(1, 6)]));
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        7, 1, 1);
                    myMat = [eye(7), -eye(7)];
                    ansEaEllMat = diag([13 ^ 2, 13 * 16 * ones(1, 6)]);
                    ansEaEllVec = self.createObjectArray([1, 2], @(varargin)self.createEll(varargin{:}), ...
                        13 * ones(7, 1), ansEaEllMat, 2);
                    ansIaEllMat = diag([13 ^ 2, (realsqrt(2.5) + 12) ^ 2 * ones(1, 6)]);
                    ansIaEllVec = self.createObjectArray([1, 2], @(varargin)self.createEll(varargin{:}), ...
                ...
                case 3
                    ...
                    my1Ell = self.createEll(eye(1));
                    my2Ell = self.createEll(1, 0.25);
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        1, 1, 1);
                    myMat = [1, -1];
                    ansEllMat = diag(6.5 ^ 2);
                    ansEllVec = self.createObjectArray([1, 2], @(varargin)self.createEll(varargin{:}), ...
                        -1, ansEllMat, 2);
                    varargout{1} = my1Ell;
                    varargout{2} = my2Ell;
                    varargout{3} = myEllArray;
                ...
                case 5
                    ...
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        7, 1, 1);
                    myEllArray(1, 2, 1, 3, 2, 1) = self.createEll(5 * ones(7, 1), ...
                        diag(9 * ones(1, 7)));
                    myMat = [eye(7), -eye(7)];
                    ansEllMat = diag(14 ^ 2 * ones(1, 7));
                    ansEllVec = self.createObjectArray([1, 14], @(varargin)self.createEll(varargin{:}), ...
                ...
                case 6
                    ...
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        1, 1, 1);
                    myEllArray(1, 1, 1, 1, 1, 2, 1, 1, 1) = self.createEll(-1, 0.25);
                    myMat = [1, -1];
                ...
                case 7
                    ...
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        10, 1, 1);
                    myEll = self.createEll(diag( 4 * ones(1, 10)));
                    myMat = eye(10);
                ...
                case 8
                    arraySize = [1, 2, 1, 3, 2, 1];
                    myEllArray = self.createObjectArray(arraySize, @(varargin)self.createEll(varargin{:}), ...
                        -3 * ones(7, 1), diag([4, ones(1, 6)]), 2);
                    myEll = self.createEll(10 * ones(7, 1), diag(9 * ones(1, 7)));
                    myMat = [eye(7), -eye(7)];
                    ansEaEllMat = diag([21 ^ 2, 9 ^ 2 ...
                        * ones(1, 6)]);
                    ansEaEllVec = self.createObjectArray([1, 14], @(varargin)self.createEll(varargin{:}), ...
                        -46 * ones(7, 1), ansEaEllMat, 2);
                    ans1IaEllMat = diag( 0.875 * 12 ^ 2 * [ 4, ones(1, 6)] - 63);
                    ans2IaEllMat = diag( 0.75 * 12 ^ 2 * [ 4, ones(1, 6)] - 27);
                    ansIaEllVec = self.createObjectArray([1, 14], @(varargin)self.createEll(varargin{:}), ...
                        -46 * ones(7, 1), ans2IaEllMat, 2);
                    ansIaEllVec(1) = self.createEll(-46 * ones(7, 1), ans1IaEllMat);
                    ansIaEllVec(8) = self.createEll(-46 * ones(7, 1), ans1IaEllMat);
                    varargout{1} = myEllArray;
                    varargout{2} = myEll;
                    varargout{3} = myMat;
        ...
        function [varargout] = createTypicalHighDimArray(self, flag)
            switch flag
                case 1
                    ...
                    my1Ell = self.createEll(eye(100));
                    my2Ell = self.createEll(ones(100, 1), diag( 0.25 * ones(1, 100)));
                    myEllArray = self.createObjectArray(arraySize, @(x)self.createEll(eye(x)), ...
                        100, 1, 1);
                    myMat = [eye(5); zeros(95, 5)];
                    ansEllMat = diag(9.5 ^ 2 * ones(1, 100));
                    ansEllVec = self.createObjectArray([1, 5], @(varargin)self.createEll(varargin{:}), ...
        ...
        function checkMinkmpEaAndMinkmpIa(self, isMinkmpEa)
            ...
            function testError(flag)
                ...
                if isMinkmpEa
                    self.runAndCheckError...
                        ('test1EllArray.minkmp_ea(self.createEll(eye(3)), test2EllArray, eye(3))', ...
                        'wrongInput');
                    self.runAndCheckError...
                        ('self.createEll(eye(3)).minkmp_ea(test1EllArray, test2EllArray, eye(3))', ...
                        'wrongInput');
                    if (flag ~= 10)
                        self.runAndCheckError...
                            ('self.createEll(eye(3)).minkmp_ea(self.createEll(eye(3)), test1EllArray, eye(3))', ...
                            errorStr);
                    end
                else
                    self.runAndCheckError...
                        ('test1EllArray.minkmp_ia(self.createEll(eye(3)), test2EllArray, eye(3))', ...
                        'wrongInput');
                    self.runAndCheckError...
                        ('self.createEll(eye(3)).minkmp_ia(test1EllArray, test2EllArray, eye(3))', ...
                        'wrongInput');
                    if (flag ~= 10)
                        self.runAndCheckError...
                            ('self.createEll(eye(3)).minkmp_ia(self.createEll(eye(3)), test1EllArray, eye(3))', ...
                            errorStr);
                    end
                end
            end
        ...
        function checkMinkpmEaAndMinkpmIa(self, isMinkpmEa)
            ...
            function testError(flag)
                ...
                if (flag == 10) || (flag == 13)
                    if isMinkpmEa
                        self.runAndCheckError...
                            ('test1EllArray.minkpm_ea(self.createEll(eye(3)), eye(3))', ...
                            errorStr);
                    else
                        self.runAndCheckError...
                            ('test1EllArray.minkpm_ia(self.createEll(eye(3)), eye(3))', ...
                            errorStr);
                    end
                else
                    if isMinkpmEa
