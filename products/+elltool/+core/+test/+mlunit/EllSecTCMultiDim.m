    ...
    function testError(flag)
        ...
        if isMinkmpEa
            self.runAndCheckError...
                ...
                'wrongInput');
            unitBallObj=ell_unitball(3);
            self.runAndCheckError...
                ('unitBallObj.minkmp_ea(test1EllArray, test2EllArray, eye(3))', ...
                'wrongInput');
            if (flag ~= 10)
                unitBallObj=ell_unitball(3); %#ok<*NASGU>
                self.runAndCheckError...
                    ('unitBallObj.minkmp_ea(ell_unitball(3), test1EllArray, eye(3))', ...
                    errorStr);
            end
        else
            self.runAndCheckError...
            ...
            if (flag ~= 10)
                ballObj=ell_unitball(3);
                self.runAndCheckError...
                    ('ballObj.minkmp_ia(ell_unitball(3), test1EllArray, eye(3))', ...
                    errorStr);
            end
        end
    end
