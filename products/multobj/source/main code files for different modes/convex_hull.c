...
{ //mex-function for ellipsoid's approx
	...
	float*points,*controlParams,*bVec,*discrVec;
	float **Amat,**vertMat;
	...
	if (nrhs !=6) {
		...
		calcEllipsoidApprox(size,indProjVec, improveDirectVec, num_points, points, Amat, bVec, vertMat, discrVec, controlParams);
 }
