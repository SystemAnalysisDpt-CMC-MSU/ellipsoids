!> Базовый сетап: пружинки с одним управлением на последнем звене.
subroutine setup_approximation(ea)
    ...
    call get_program_parameter("nv", springs%nv, 0)
    ...
    call get_program_parameter("unilateral", unilateral, .false.)
    if( unilateral ) ea%pc = 1
    do j = 1,ea%nu
        ea%P(j, j) = rP
    end do

    call get_program_parameter("rQ", rQ, 0D0)
    do j = 1,ea%nv
        ea%Q(j, j) = rQ
    end do
...
subroutine disturbance(cl, nx, nu, nv, t, x, u, v)
    ...
    integer, intent(in), value :: nx, nu, nv
