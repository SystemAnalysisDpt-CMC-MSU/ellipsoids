...
function l_check_ell_struct(E, N, typeStr)
%
...
if iscell(Q)
    ...
    if isa(Q, 'double')
        checkvar(Q,@(x)gras.la.ismatsymm(x),'errorTag',...
            strcat('linsys:value:',typeStr,':shape'),...
            'errorMessage','shape matrix must be symmetric.');
        checkmultvar('gras.la.ismatposdef(x1,x2)',2,Q, absTol,...
            'errorTag', strcat('linsys:value:',typeStr,':shape'),...
            'errorMessage',...
